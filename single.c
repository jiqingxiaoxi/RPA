...
float tm(char seq[],float deltah[],float deltas[],int length)
{
	...
	result=1000.0*total_deltah/(total_deltas-0.6656578*(length-1)-36.1585064)-273.15;
	return result;
}
...
float GContent(char primer[],int length)
{
	int i,G;
	G=0;
	for(i=0;i<length;i++)
	...
	return 1.0*G/length*100;
}

int check_G_C_Count(char primer[],int length)
{
	int i,G,C;
	G=0;
	C=0;
...
//primers
	for(circle=0;circle<length;circle++)
	{
		...
		//	minus=dimer(rev,i);
			...
			if(plus+minus==0)
                                continue;
			fprintf(Primer,"pos:%d\tlength:%d\t+:%d\t-:%d\t%0.2f\n",circle,i,plus,minus,Tm);
			Num[0]++;
		}
		if(flag[3]==1)
			continue;

		for(i=1;i<=5;i++) //dis between two Ts
		{
			if(circle+44+i>length)
				break;
			check=words(seq,circle,(45+i));
			if(check==0)
				break;
			memset(primer,'\0',60);
		//probe length=45+i
			generate(seq,primer,circle,(45+i));
			check=check_long_ploy(primer,(45+i));
			if(check==0)
				break;
		//Probe-plus
	                if((seq[circle+30-1]=='T')&&(seq[circle+30+i]=='T'))
			{
				check=check_G_C_Count(primer,(45+i));
				if(check==1)
					Tm=tm(primer,deltah,deltas,(45+i));
				if((check==1)&&(flag[4]==0))
				{
					secondary=thal(primer,primer,stackEntropies,stackEnthalpies,stackint2Entropies,stackint2Enthalpies,dangleEntropies3,dangleEnthalpies3,dangleEntropies5,dangleEnthalpies5,hairpinLoopEntropies,interiorLoopEntropies,bulgeLoopEntropies,hairpinLoopEnthalpies,interiorLoopEnthalpies,bulgeLoopEnthalpies,tstackEntropies,tstackEnthalpies,tstack2Entropies,tstack2Enthalpies,triloopEntropies1,triloopEnthalpies1,tetraloopEntropies1,tetraloopEnthalpies1,triloopEntropies2,triloopEnthalpies2,tetraloopEntropies2,tetraloopEnthalpies2,numTriloops,numTetraloops,atpS,atpH,1);
					if(secondary>Tm-20)
				...
				if(check==1)
				{
					G_content=GContent(primer,(45+i));
					fprintf(Probe,"pos:%d\tlength:%d\t+:1\t-:0\t%0.2f\t%0.2f\n",circle,(45+i),Tm,G_content);
					Num[1]++;
				}
			}
		//Probe-minus
			if((seq[circle+15-1]=='A')&&(seq[circle+15+i]=='A'))
			{
                        //probe length=45+i
				reverse(primer,rev,(45+i));
				check=check_G_C_Count(rev,(45+i));
				if(check==0)
					continue;
				Tm=tm(rev,deltah,deltas,(45+i));
	                        if(flag[4]==0)
				{
					secondary=thal(rev,rev,stackEntropies,stackEnthalpies,stackint2Entropies,stackint2Enthalpies,dangleEntropies3,dangleEnthalpies3,dangleEntropies5,dangleEnthalpies5,hairpinLoopEntropies,interiorLoopEntropies,bulgeLoopEntropies,hairpinLoopEnthalpies,interiorLoopEnthalpies,bulgeLoopEnthalpies,tstackEntropies,tstackEnthalpies,tstack2Entropies,tstack2Enthalpies,triloopEntropies1,triloopEnthalpies1,tetraloopEntropies1,tetraloopEnthalpies1,triloopEntropies2,triloopEnthalpies2,tetraloopEntropies2,tetraloopEnthalpies2,numTriloops,numTetraloops,atpS,atpH,1);
					if(secondary>Tm-20)
				...
				G_content=GContent(rev,(45+i));
				fprintf(Probe,"pos:%d\tlength:%d\t+:0\t-:1\t%0.2f\t%0.2f\n",circle,(45+i),Tm,G_content);
				Num[1]++;
			}
		}
	}
...
void usage()
{
	...
	printf("  -out <primer_probe>\n");
	printf("    output the candidate single primers and probes\n");
	printf("  -dir <directory>\n");
	printf("    the directory for output file\n");
	printf("    default: current directory\n");
        printf("  -NoExo\n");
	printf("    don't identifiy candidate exo probe\n");
	printf("    default:identifiy candidate exo probe\n");
	printf("  -NoCheck\n");
	printf("    don't check secondary structure of primers or probes\n");
	printf("    default: check secondary structure\n");
...
void main(int argc, char **argv) 
{
	...
	char *store_path,*prefix,*tm_path,*curren_path,*input,*par_path,*seq,*temp;
	FILE *fp,*Primer,*Probe;
	int i,flag[10],length,numTriloops,numTetraloops,Num[3];
	float deltah[16],deltas[16],temp1,temp2;
	struct stat statbuf;
//flag: 0:input; 1: out_prefix; 2: dir; 3: NoExo; 4: No secondary structure; 5: path for secondary structure
        time_t start,end;
...
//input Tm parameter
	...
	Num[0]=0; //primer
	Num[1]=0; //probe
	candidate_primer(seq,flag,Primer,Probe,Num,deltah,deltas,length,stackEntropies,stackEnthalpies,stackint2Entropies,stackint2Enthalpies,dangleEntropies3,dangleEnthalpies3,dangleEntropies5,dangleEnthalpies5,hairpinLoopEntropies,interiorLoopEntropies,bulgeLoopEntropies,hairpinLoopEnthalpies,interiorLoopEnthalpies,bulgeLoopEnthalpies,tstackEntropies,tstackEnthalpies,tstack2Entropies,tstack2Enthalpies,triloopEntropies1,triloopEnthalpies1,tetraloopEntropies1,tetraloopEnthalpies1,triloopEntropies2,triloopEnthalpies2,tetraloopEntropies2,tetraloopEnthalpies2,numTriloops,numTetraloops,atpS,atpH);
	free(seq);
	printf("There are %d candidate primers.\n",Num[0]);
	fclose(Primer);
	if(flag[3]==0)
	{
		printf("There are %d candidate exo probes.\n",Num[1]);
		fclose(Probe);
	}
	//check
	if(Num[0]<2)
		printf("Warning: there don't have enough primers(>=2).\n");
	if(flag[3]==0 && Num[1]<1)
		printf("Warning: there don't have any probes.\n");

	end=time(NULL);
        printf("It takes %d seconds to identify candidate single primer or probe regions.\n",(int)difftime(end,start));
